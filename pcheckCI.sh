...
then	# Le travail est reporté à la prochaine exécution si le lock de package check est présent.
	...
	then	# Si la liste de test n'est pas vide
		...
		"$script_dir/package_check/package_check.sh" --bash-mode $APP &	# Exécute package_check sur la première adresse de la liste, et passe l'exécution en arrière plan.
